...
import { UserPlus, ArrowLeft, Check, Shield, Eye, EyeOff } from 'lucide-react'
...
function SimpleCaptcha({ onVerify }: { onVerify: (ok: boolean) => void }) {
  ...
  return (
    <div className="flex items-center gap-3 bg-gray-700/50 rounded-xl p-4">
      ...
      <input type="number" value={answer} onChange={(e) => check(e.target.value)}
        className="w-20 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-center focus:ring-2 focus:ring-cyan-400" placeholder="?" />
...
export default function RegisterForm({ onBack }: RegisterProps) {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    const { data, error: signUpError } = await supabase.auth.signUp({
      ...
      options: {
        data: { full_name: `${form.name} ${form.lastName}` },
    ...
    if (data?.user) {
      ...
      try {
        await fetch('/api/send-email', {
          ...
          body: JSON.stringify({ type: 'welcome', data: { name: `${form.name} ${form.lastName}`, email: form.email } }),
        })
      } catch {}
    }
    ...
    return (
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-400">
          Te enviamos un email de bienvenida a <span className="text-cyan-400">{form.email}</span>.
          Ya podés iniciar sesión y reservar pistas.
        </p>
        <button onClick={onBack} className="mt-4 px-6 py-3 bg-cyan-500 hover:bg-cyan-400 text-white font-bold rounded-xl transition-all">
          Volver al Login
        </button>
      </div>
    )
  ...
  return (
    <form onSubmit={handleRegister} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
            className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white focus:ring-2 focus:ring-cyan-400" placeholder="Juan" required />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-1">Apellido *</label>
          <input value={form.lastName} onChange={(e) => setForm({ ...form, lastName: e.target.value })}
            className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white focus:ring-2 focus:ring-cyan-400" placeholder="García" required />
        </div>
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-300 mb-1">Email *</label>
        <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
          className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white focus:ring-2 focus:ring-cyan-400" placeholder="email@ejemplo.com" required />
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-300 mb-1">Teléfono *</label>
        <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}
          className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white focus:ring-2 focus:ring-cyan-400" placeholder="+34 600 000 000" required />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <div className="relative">
            <input type={showPassword ? 'text' : 'password'} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })}
              className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white focus:ring-2 focus:ring-cyan-400 pr-10" placeholder="Min. 6" required />
        ...
        <div>
          ...
          <div className="relative">
            <input type={showConfirm ? 'text' : 'password'} value={form.confirmPassword} onChange={(e) => setForm({ ...form, confirmPassword: e.target.value })}
              className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white focus:ring-2 focus:ring-cyan-400 pr-10" placeholder="Repetir" required />
      ...
      {error && <div className="bg-red-500/20 border border-red-500 text-red-300 px-4 py-3 rounded-xl text-sm">{error}</div>}
