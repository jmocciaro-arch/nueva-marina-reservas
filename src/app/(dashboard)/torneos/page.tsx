...
import type { Tournament, TournamentCategory, TournamentRegistration } from '@/lib/types'
import { Trophy, Plus, Edit2, Trash2, X, Save, Users, Download, Eye, Swords } from 'lucide-react'
...
interface TournamentForm {
  name: string; start_date: string; end_date: string; status: string; format: string; categories: string[]
}
const defaultForm: TournamentForm = {
  name: '', start_date: '', end_date: '', status: 'upcoming', format: 'elimination', categories: ['5ta', '6ta', '7ma', 'Libre']
}
...
export default function TorneosPage() {
  ...
  function openEdit(t: Tournament) {
    setEditingId(t.id)
    setForm({ name: t.name, start_date: t.start_date, end_date: t.end_date || '', status: t.status, format: t.format, categories: [] })
    setShowModal(true)
  }

  async function handleSave() {
    setLoading(true)
    const data = { name: form.name, start_date: form.start_date, end_date: form.end_date || null, status: form.status, format: form.format }
    if (editingId) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <h1 className="text-3xl font-bold text-white flex items-center gap-3">
          <Trophy className="text-yellow-400" size={32} /> Torneos & Ligas
        </h1>
      ...
      {tournaments.length === 0 ? (
        <div className="bg-gray-800 rounded-2xl p-12 border border-gray-700 text-center">
          ...
          <p className="text-gray-400 text-lg">No hay torneos creados. Creá el primero!</p>
        </div>
      ) : (
          ...
          {tournaments.map((tournament) => (
            <div key={tournament.id} className="bg-gray-800 rounded-2xl border border-gray-700 overflow-hidden hover:border-yellow-500/30 transition-all">
              <div className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 p-6">
                ...
                <h3 className="text-white font-bold text-xl mt-3">{tournament.name}</h3>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto border border-gray-700 shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-gray-700">
              <h2 className="text-xl font-bold text-white">{editingId ? 'Editar Torneo' : 'Nuevo Torneo'}</h2>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white text-lg focus:ring-2 focus:ring-yellow-400" placeholder="Ej: Torneo Apertura 2026" />
              </div>
